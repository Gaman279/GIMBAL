...
int main(int argc, char **argv) {
  ...
  const double k_vertical_cubic_p = 1.0;  // P constant of the cubic vertical PID.
  ...
  while (wb_robot_step(timestep) != -1) {
    ...
    const double vertical_input = k_vertical_p * clamped_difference_altitude;

    // Compute the cubic control for the vertical input.
    const double clamped_difference_altitude_cubic = CLAMP(target_altitude - altitude + k_vertical_offset, -1.0, 1.0);
    const double cubic_input = k_vertical_cubic_p * pow(clamped_difference_altitude_cubic, 3.0);

    // Actuate the motors taking into consideration all the computed inputs.
    const double front_left_motor_input = k_vertical_thrust + vertical_input - roll_input + pitch_input - yaw_input + cubic_input;
    const double front_right_motor_input = k_vertical_thrust + vertical_input + roll_input + pitch_input + yaw_input + cubic_input;
    const double rear_left_motor_input = k_vertical_thrust + vertical_input - roll_input - pitch_input + yaw_input + cubic_input;
    const double rear_right_motor_input = k_vertical_thrust + vertical_input + roll_input - pitch_input - yaw_input + cubic_input;
